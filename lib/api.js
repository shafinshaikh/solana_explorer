...
async function fetchFromHelius(method, params, network) {
...
export async function fetchAccountInfo(address, network) {
  const accountInfo = await fetchFromHelius('getAccountInfo', [address], network);
  const balance = await fetchFromHelius('getBalance', [address], network);
  const tokenAccounts = await fetchFromHelius('getTokenAccountsByOwner', [address, { programId: 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA' }], network);
...
export async function fetchTransactionDetails(signature, network) {
  const tx = await fetchFromHelius('getTransaction', [signature], network);
...
export async function fetchNetworkStats(network) {
  const [supply, recentPerformanceSamples, epochInfo, inflation] = await Promise.all([
    fetchFromHelius('getSupply', [], network),
    fetchFromHelius('getRecentPerformanceSamples', [30], network),
    fetchFromHelius('getEpochInfo', [], network),
    fetchFromHelius('getInflationRate', [], network),
  ]);

  const solPrice = await fetchSolPrice();  
  console.log("supply: ", supply);
  const totalSupply = supply.value.total / 1e9;
  ...
  return {
    ...
    // totalStake: epochInfo.totalActivatedStake / 1e9,
...
export async function fetchRecentTransactions(address, limit = 10, network) {
  const signatures = await fetchFromHelius('getSignaturesForAddress', [address, { limit }], network);
  const transactions = await Promise.all(signatures.map(sig => fetchTransactionDetails(sig.signature, network)));
  return transactions;
}
...
export async function fetchNFTs(address, network) {
  const nfts = await fetchFromHelius('getAssetsByOwner', [address, { page: 1, limit: 1000 }], network);
